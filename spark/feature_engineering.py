"""
Feature Engineering
Reads clean Parquet, adds computed features, and writes the enriched feature Parquet
for use by the MLlib training pipeline and agent bridge.
"""
import os
import sys
import time
...
def run_features(input_path, output_path, spark=None):
    ...
    try:
        ...
        if record_count == 0:
            ...
            # Fall back to minimal sample data so the pipeline can continue end-to-end.
            sample_data = [
    ...
    # ── 3. Is Emergency — ambulance or police vehicle ─────────────────────────
    df = df.withColumn(
        "is_emergency",
    ...
    # ── 5. Hour Bin — time of day category ───────────────────────────────────
    df = df.withColumn(
        "hour_bin",
    ...
    # ── 6. Zone Avg Congestion — rolling avg per zone ────────────────────────
    ...
    # ── 7. Zone Avg Speed — rolling avg per zone ─────────────────────────────
    df = df.withColumn(
        "zone_avg_speed",
    ...
    # ── 9. Severity Label for ML (derived from risk_score) ───────────────────
    df = df.withColumn(
        "severity_label",
    ...
    # ── 10. Severity Index — numeric label for MLlib ─────────────────────────
    df = df.withColumn(
        "severity_index",
